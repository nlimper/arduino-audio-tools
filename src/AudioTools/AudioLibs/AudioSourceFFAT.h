...
namespace audio_tools {
	...
	/**
	 * @brief ESP32 AudioSource for AudioPlayer using an the FFAT file system
	 * @ingroup player
	 * @author Phil Schatzmann
	 * @copyright GPLv3
	 */
	class AudioSourceFFAT : public AudioSource {
	public:
		/// Default constructor
		AudioSourceFFAT(const char *startFilePath = "/", const char *ext = ".mp3") {
		...
		virtual Stream *selectStream(int index) override {
			...
			file = FFat.open(file_name, "r");
			return file ? &file : nullptr;
		}

		virtual Stream *selectStream(const char *path) override {
			file.close();
			file = FFat.open(path, "r");
			file_name = file.name();
